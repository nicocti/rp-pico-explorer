...
use board::{Button, PicoExplorer, XOSC_CRYSTAL_FREQ};
use core::fmt::Write;
...
use panic_halt as _;
use rp2040_hal::{adc::Adc, clocks::*, entry, pac, watchdog::Watchdog, Sio};
...
fn main() -> ! {
    ...
    loop {
        ...
        // Write to buffer
        writeln!(&mut buf, "Bonjour {}", if even { '|' } else { '-' }).unwrap();
